...
const CommentItem = ({ comment, activeUser }) => {
    ...
    useEffect(() => {
        ...
        const getCommentLikeStatus = async () => {
            ...
            try {
                const { data } = await axios.post(`${server}comment/${comment_id}/getCommentLikeStatus`, { activeUser }, {
                    headers: {
    ...
    const handleCommentLike = async () => {
        ...
        try {
            const { data } = await axios.post(`${server}comment/${comment_id}/like`, { activeUser }, {
                headers: {
                    "Content-Type": "application/json",
                    authorization: `Bearer ${localStorage.getItem("authToken")}`,
                },
    ...
    return (
        ...
        <div className='comment-item'>
            <div className="comment-top-block">
                ...
                <section>
                    <img src={`${server}userPhotos/${comment.author.photo}`} alt={comment.author.username} width="35" />
